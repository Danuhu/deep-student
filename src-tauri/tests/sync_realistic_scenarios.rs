...
impl SimulatedCloudStore {
    ...
    pub fn upload_changes(&self, device_id: &str, batch: &[Value]) {
        ...
        self.changes.lock().unwrap().entry(device_id.to_string()).or_default().push(encoded);
        *self.versions.lock().unwrap().entry(device_id.to_string()).or_insert(0) += 1;
    }
    ...
    pub fn download_changes(&self, exclude_device_id: &str) -> Vec<Value> {
        ...
        for (dev_id, batches) in changes.iter() {
            if dev_id == exclude_device_id { continue; }
    ...
    pub fn get_version(&self, device_id: &str) -> u64 {
        self.versions.lock().unwrap().get(device_id).copied().unwrap_or(0)
    }
...
impl SyncDevice {
    ...
    pub fn setup_vfs_schema(&self) {
        self.vfs_db.execute_batch(include_str!("../migrations/vfs/V20260130__init.sql")).unwrap();
        self.vfs_db.execute_batch(include_str!("../migrations/vfs/V20260131__add_change_log.sql")).unwrap();
        let _ = self.vfs_db.execute_batch(include_str!("../migrations/vfs/V20260210__add_answer_submissions.sql"));
        // V20260211: fix questions change_log record_id (exam_id → id)
        let _ = self.vfs_db.execute_batch(include_str!("../migrations/vfs/V20260211__fix_change_log_record_id.sql"));
        let _ = self.vfs_db.execute_batch(include_str!("../migrations/vfs/V20260308__add_todo_tables.sql"));
        let _ = self.vfs_db.execute_batch(include_str!("../migrations/vfs/V20260310__add_pomodoro.sql"));
        let _ = self.vfs_db.execute_batch(include_str!("../migrations/vfs/V20260523__add_missing_sync_coverage.sql"));
    }

    pub fn setup_chat_v2_schema(&self) {
        self.chat_v2_db.execute_batch(include_str!("../migrations/chat_v2/V20260130__init.sql")).unwrap();
        self.chat_v2_db.execute_batch(include_str!("../migrations/chat_v2/V20260131__add_change_log.sql")).unwrap();
        let _ = self.chat_v2_db.execute_batch(include_str!("../migrations/chat_v2/V20260204__session_groups.sql"));
        let _ = self.chat_v2_db.execute_batch(include_str!("../migrations/chat_v2/V20260523__add_missing_sync_coverage.sql"));
    }

    pub fn setup_mistakes_schema(&self) {
        self.mistakes_db.execute_batch(include_str!("../migrations/mistakes/V20260130__init.sql")).unwrap();
        self.mistakes_db.execute_batch(include_str!("../migrations/mistakes/V20260131__add_change_log.sql")).unwrap();
        let _ = self.mistakes_db.execute_batch(include_str!("../migrations/mistakes/V20260523__add_missing_sync_coverage.sql"));
    }
}
...
pub fn create_test_folder_item(device: &SyncDevice, id: &str, folder_id: &str, item_type: &str, item_id: &str) {
    let ts = Utc::now().timestamp_millis();
    device.vfs_db.execute(
...
pub fn create_test_todo_item(device: &SyncDevice, id: &str, todo_list_id: &str, title: &str, parent_id: Option<&str>) {
    let ts = Utc::now().to_rfc3339();
    device.vfs_db.execute(
...
pub fn create_test_essay(device: &SyncDevice, id: &str, resource_id: &str, title: &str, score: Option<i64>) {
    let ts = Utc::now().to_rfc3339();
    device.vfs_db.execute(
...
pub fn create_test_answer_submission(device: &SyncDevice, id: &str, question_id: &str, user_answer: &str, is_correct: i64) {
    let ts = Utc::now().to_rfc3339();
    device.vfs_db.execute(
...
pub fn create_test_chat_message(device: &SyncDevice, id: &str, session_id: &str, role: &str, timestamp: i64) {
    device.chat_v2_db.execute(
...
pub fn create_test_chat_attachment(device: &SyncDevice, id: &str, message_id: &str, name: &str, mime_type: &str, size: i64) {
    let ts = Utc::now().to_rfc3339();
    device.chat_v2_db.execute(
...
pub fn create_test_workspace_index(device: &SyncDevice, workspace_id: &str, name: &str, creator_session_id: &str) {
    let ts = Utc::now().to_rfc3339();
    device.chat_v2_db.execute(
...
pub fn create_test_chat_message_mistakes(device: &SyncDevice, mistake_id: &str, role: &str, content: &str) {
    let ts = Utc::now().to_rfc3339();
    device.mistakes_db.execute(
...
pub fn create_test_review_session(device: &SyncDevice, id: &str, title: &str, start_date: &str, end_date: &str) {
    let ts = Utc::now().to_rfc3339();
    device.mistakes_db.execute(
...
pub fn create_test_anki_card(device: &SyncDevice, id: &str, task_id: &str, front: &str, back: &str, tags_json: &str) {
    let ts = Utc::now().to_rfc3339();
    device.mistakes_db.execute(
...
fn collect_pending_changes(db: &Connection) -> Vec<Value> {
    ...
    for (change_id, table_name, record_id, operation, changed_at) in &rows {
        let op = match operation.as_str() {
            "INSERT" => "Insert", "UPDATE" => "Update", "DELETE" => "Delete", _ => continue,
        };
        let data = if *operation != "DELETE" { read_row_data(db, table_name, record_id) } else { None };
        changes.push(json!({
    ...
    if !rows.is_empty() {
        db.execute("UPDATE __change_log SET sync_version = ?1 WHERE sync_version = 0",
            params![Utc::now().timestamp()]).unwrap();
    }
    changes
}

fn read_row_data(db: &Connection, table_name: &str, record_id: &str) -> Option<Value> {
    let col_names: Vec<String> = db.prepare(&format!("SELECT name FROM pragma_table_info('{}')", table_name))
        .ok()?.query_map([], |r| r.get(0)).ok()?.filter_map(|r| r.ok()).collect();
    ...
    if let Ok(mut stmt) = db.prepare(&sql_id) {
        if let Ok(result) = stmt.query_row(params![record_id], |row| read_row_to_json(row, &col_names)) {
            return Some(result);
        }
    }
    ...
    for (ct, pk_cols) in composite_tables {
        if table_name == *ct {
            ...
            if parts.len() == pk_cols.len() {
                let where_clause: Vec<String> = pk_cols.iter().enumerate()
                    .map(|(i, col)| format!("\"{}\" = '{}'", col, parts[i]))
                    .collect();
                let sql = format!("SELECT * FROM \"{}\" WHERE {}", table_name, where_clause.join(" AND "));
    ...
    let pk_candidates = ["workspace_id", "session_id", "version_id", "exam_id", "document_id", "item_type", "key"];
    for pk in &pk_candidates {
        if !col_names.iter().any(|c| c == pk) { continue; }
...
fn read_row_to_json(row: &rusqlite::Row, col_names: &[String]) -> rusqlite::Result<Value> {
    ...
    for (i, name) in col_names.iter().enumerate() {
        ...
        match val {
            rusqlite::types::ValueRef::Null => { map.insert(name.clone(), Value::Null); }
            rusqlite::types::ValueRef::Integer(v) => { map.insert(name.clone(), json!(v)); }
            rusqlite::types::ValueRef::Real(v) => { map.insert(name.clone(), json!(v)); }
            rusqlite::types::ValueRef::Text(v) => { map.insert(name.clone(), json!(String::from_utf8_lossy(v))); }
            rusqlite::types::ValueRef::Blob(_) => { map.insert(name.clone(), Value::Null); }
        }
    }
    Ok(Value::Object(map))
}
...
fn apply_changes_to_db(db: &Connection, changes: &[Value]) -> usize {
    ...
    for change in changes {
        ...
        if table_name.starts_with("__") || table_name.starts_with("sqlite_") { continue; }
        ...
        match operation {
            "Insert" | "Update" => {
                if let Some((cols, vals)) = &col_values {
                    ...
                    if cols.contains(&"id") {
                        let sql = format!("INSERT OR IGNORE INTO \"{}\" ({}) VALUES ({})", table_name, col_list, val_list);
            ...
            "Delete" => {
                let ts = Utc::now().to_rfc3339();
                let sql_id = format!("UPDATE \"{}\" SET deleted_at = '{}', updated_at = '{}' WHERE id = '{}'", table_name, ts, ts, record_id.replace('\'', "''"));
                ...
                if rows == 0 {
                    for pk in &["workspace_id", "session_id"] {
                        ...
                        if db.execute(&sql_pk, []).unwrap_or(0) > 0 { break; }
                    }
                }
                applied += 1;
            }
...
pub fn full_sync_cycle_all(device_a: &SyncDevice, device_b: &SyncDevice, cloud: &SimulatedCloudStore) -> (usize, usize) {
...
pub fn full_sync_cycle(device_a: &SyncDevice, device_b: &SyncDevice, cloud: &SimulatedCloudStore) -> (usize, usize) {
    let changes_a = collect_pending_changes(&device_a.vfs_db);
    if !changes_a.is_empty() { cloud.upload_changes(&device_a.device_id, &changes_a); }
    ...
    if !changes_b.is_empty() { cloud.upload_changes(&device_b.device_id, &changes_b); }
...
pub fn full_sync_cycle_chat_v2(device_a: &SyncDevice, device_b: &SyncDevice, cloud: &SimulatedCloudStore) -> (usize, usize) {
...
pub fn full_sync_cycle_mistakes(device_a: &SyncDevice, device_b: &SyncDevice, cloud: &SimulatedCloudStore) -> (usize, usize) {
...
pub fn verify_devices_converged(device_a: &SyncDevice, device_b: &SyncDevice, table_name: &str, db_name: &str) -> bool {
    let db_a = match db_name { "vfs" => &device_a.vfs_db, "chat_v2" => &device_a.chat_v2_db, "mistakes" => &device_a.mistakes_db, _ => return false };
    let db_b = match db_name { "vfs" => &device_b.vfs_db, "chat_v2" => &device_b.chat_v2_db, "mistakes" => &device_b.mistakes_db, _ => return false };
    let count_a: i64 = db_a.query_row(&format!("SELECT COUNT(*) FROM \"{}\"", table_name), [], |r| r.get(0)).unwrap_or(-1);
    let count_b: i64 = db_b.query_row(&format!("SELECT COUNT(*) FROM \"{}\"", table_name), [], |r| r.get(0)).unwrap_or(-1);
    count_a == count_b
}

pub fn row_count(db: &Connection, table: &str) -> i64 {
    db.query_row(&format!("SELECT COUNT(*) FROM \"{}\" WHERE deleted_at IS NULL", table), [], |r| r.get(0)).unwrap_or(0)
}
...
fn s01_single_device_creates_note_syncs_other_receives() {
    ...
    setup_all_schemas(&device_a); setup_all_schemas(&device_b);
...
fn s02_update_note_title_sync_propagates() {
    ...
    let ts = Utc::now().to_rfc3339();
    device_a.vfs_db.execute("UPDATE notes SET title = 'Updated', updated_at = ?1 WHERE id = 'note_002'", params![ts]).unwrap();
    let (b_applied, _) = full_sync_cycle(&device_a, &device_b, &cloud);
    log_sync(&format!("S02: B applied {}", b_applied));
    assert_eq!(get_column(&device_b.vfs_db, "notes", "note_002", "title").as_deref(), Some("Updated"));
    let _log = dump_sync_log();
}
...
fn s03_delete_note_tombstone_sync_propagates() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    let ts = Utc::now().to_rfc3339();
    device_a.vfs_db.execute("UPDATE notes SET deleted_at = ?1, updated_at = ?1 WHERE id = 'note_003'", params![ts]).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
...
fn s04_create_50_notes_bulk_sync() {
    ...
    let cloud = SimulatedCloudStore::new();
    for i in 0..50 {
        create_test_resource(&device_a, &format!("res_{:03}", i), "note", "data", &format!("h_{:03}", i));
        create_test_note(&device_a, &format!("note_{:03}", i), &format!("res_{:03}", i), &format!("Note {}", i));
    }
    full_sync_cycle(&device_a, &device_b, &cloud);
...
fn s05_create_resource_then_note_fk_order() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "notes", "note_005", "resource_id").as_deref(), Some("res_005"));
    let _log = dump_sync_log();
}
...
fn s06_update_resource_data_verify_note_sees_new_data() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE resources SET data = 'new' WHERE id = 'res_006'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "resources", "res_006", "data").as_deref(), Some("new"));
    let _log = dump_sync_log();
}
...
fn s07_create_question_linked_to_exam_sync_both() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "questions", "q_007", "content").as_deref(), Some("2+2?"));
    let _log = dump_sync_log();
}
...
fn s08_create_review_plan_for_question_sync() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "review_plans", "rp_008", "question_id").as_deref(), Some("q_008"));
    let _log = dump_sync_log();
}
...
fn s09_create_folder_add_items_sync_tree() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "folder_items", "fi_009", "item_type").as_deref(), Some("note"));
    let _log = dump_sync_log();
}
...
fn s10_create_todo_list_with_items_sync_hierarchy() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "todo_items", "ti_010", "title").as_deref(), Some("Task 1"));
    let _log = dump_sync_log();
}
...
fn s11_create_pomodoro_record_linked_to_todo_item_sync() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "pomodoro_records", "pd_011", "todo_item_id").as_deref(), Some("ti_011"));
    let _log = dump_sync_log();
}
...
fn s12_create_essay_sync_with_grading() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "essays", "essay_012", "title").as_deref(), Some("Great Essay"));
    let _log = dump_sync_log();
}
...
fn s13_create_translation_sync_with_metadata() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "translations", "tr_013", "title").as_deref(), Some("EN-ZH"));
    let _log = dump_sync_log();
}
...
fn s14_create_mindmap_sync_with_settings() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "mindmaps", "mm_014", "title").as_deref(), Some("KM"));
    let _log = dump_sync_log();
}
...
fn s15_create_exam_sheet_sync_with_preview() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "exam_sheets", "exam_015", "exam_name").as_deref(), Some("Chem"));
    let _log = dump_sync_log();
}
...
fn s16_create_session_with_messages_and_blocks_full_sync() {
    ...
    let c: i64 = device_b.chat_v2_db.query_row("SELECT COUNT(*) FROM chat_v2_sessions", [], |r| r.get(0)).unwrap();
    assert_eq!(c, 1);
    let _log = dump_sync_log();
}
...
fn s17_update_message_content_streaming_block_sync() {
    ...
    device_a.chat_v2_db.execute("UPDATE chat_v2_blocks SET content = 'Updated stream' WHERE id = 'blk_017'", []).unwrap();
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let content: String = device_b.chat_v2_db.query_row("SELECT content FROM chat_v2_blocks WHERE id='blk_017'", [], |r| r.get(0)).unwrap_or_default();
...
fn s18_add_attachment_to_message_sync() {
    ...
    create_test_chat_attachment(&device_a, "att_018", "msg_018", "photo.png", "image/png", 102400);
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let name: String = device_b.chat_v2_db.query_row("SELECT name FROM chat_v2_attachments WHERE id='att_018'", [], |r| r.get(0)).unwrap();
...
fn s19_create_session_group_sync() {
    ...
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let n: String = device_b.chat_v2_db.query_row("SELECT name FROM chat_v2_session_groups WHERE id='group_019'", [], |r| r.get(0)).unwrap();
...
fn s20_link_session_to_mistake_sync() {
    ...
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let c: i64 = device_b.chat_v2_db.query_row("SELECT COUNT(*) FROM chat_v2_session_mistakes WHERE session_id='sess_020'", [], |r| r.get(0)).unwrap();
    assert_eq!(c, 1);
    let _log = dump_sync_log();
}
...
fn s21_create_workspace_index_entry_sync() {
    ...
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let n: String = device_b.chat_v2_db.query_row("SELECT name FROM workspace_index WHERE workspace_id='ws_021'", [], |r| r.get(0)).unwrap();
    assert_eq!(n, "My WS");
    let _log = dump_sync_log();
}
...
fn s22_create_chat_v2_resources_entry_sync() {
    ...
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let h: String = device_b.chat_v2_db.query_row("SELECT hash FROM resources WHERE id='res_022'", [], |r| r.get(0)).unwrap();
    assert_eq!(h, "h022");
    let _log = dump_sync_log();
}
...
fn s23_multi_model_variant_in_message_sync_variants_json() {
    ...
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let av: String = device_b.chat_v2_db.query_row("SELECT active_variant_id FROM chat_v2_messages WHERE id='msg_023'", [], |r| r.get(0)).unwrap();
    assert_eq!(av, "v1");
    let _log = dump_sync_log();
}
...
fn s24_session_compaction_sync_reference() {
    ...
    device_a.chat_v2_db.execute("UPDATE chat_v2_sessions SET summary_hash = 'chash_024' WHERE id = 'sess_024'", []).unwrap();
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let sh: String = device_b.chat_v2_db.query_row("SELECT summary_hash FROM chat_v2_sessions WHERE id='sess_024'", [], |r| r.get(0)).unwrap();
...
fn s25_large_chat_history_50_messages_bulk_sync() {
    ...
    let c: i64 = device_b.chat_v2_db.query_row("SELECT COUNT(*) FROM chat_v2_messages WHERE session_id='sess_025'", [], |r| r.get(0)).unwrap();
    assert_eq!(c, 50);
    let _log = dump_sync_log();
}
...
fn s26_create_mistake_with_analysis_images_sync() {
    ...
    let q: String = device_b.mistakes_db.query_row("SELECT user_question FROM mistakes WHERE id='mistake_026'", [], |r| r.get(0)).unwrap();
...
fn s27_add_chat_messages_to_mistake_sync_conversation() {
    ...
    let c: i64 = device_b.mistakes_db.query_row("SELECT COUNT(*) FROM chat_messages WHERE mistake_id='mistake_027'", [], |r| r.get(0)).unwrap();
    assert!(c >= 2);
    let _log = dump_sync_log();
}
...
fn s28_create_review_analysis_sync_with_mistake_ids() {
    ...
    full_sync_cycle_mistakes(&device_a, &device_b, &cloud);
    let ids: String = device_b.mistakes_db.query_row("SELECT mistake_ids FROM review_analyses WHERE id='ra_028'", [], |r| r.get(0)).unwrap();
...
fn s29_create_review_session_with_linked_mistakes_sync() {
    ...
    create_test_review_session(&device_a, "rs_029", "Calc Review", "2026-05-01", "2026-05-07");
    ...
    full_sync_cycle_mistakes(&device_a, &device_b, &cloud);
    let c: i64 = device_b.mistakes_db.query_row("SELECT COUNT(*) FROM review_session_mistakes WHERE session_id='rs_029'", [], |r| r.get(0)).unwrap();
    assert_eq!(c, 1);
    let _log = dump_sync_log();
}
...
fn s30_create_anki_card_from_document_tasks_sync() {
    ...
    create_test_anki_card(&device_a, "ac_030", "task_030", "Q: 2+2?", "A: 4", "[\"math\"]");
    full_sync_cycle_mistakes(&device_a, &device_b, &cloud);
    let front: String = device_b.mistakes_db.query_row("SELECT front FROM anki_cards WHERE id='ac_030'", [], |r| r.get(0)).unwrap();
...
fn s31_custom_anki_template_sync_as_backup() {
    ...
    let change_count: i64 = device_a.mistakes_db.query_row(
        "SELECT COUNT(*) FROM __change_log WHERE table_name='custom_anki_templates'",
        [], |r| r.get(0)).unwrap();
    assert_eq!(change_count, 0, "custom_anki_templates is BackupOnly — no change_log entries");
    // B should NOT receive the record
    let c: i64 = device_b.mistakes_db.query_row(
...
fn s43_both_devices_add_different_tags_to_question() {
    ...
    device_a.vfs_db.execute("UPDATE notes SET tags = ?1 WHERE id = ?2", params!["[\"hard\"]", "note_043"]).unwrap();
    device_b.vfs_db.execute("UPDATE notes SET tags = ?1 WHERE id = ?2", params!["[\"review\"]", "note_043"]).unwrap();
...
fn s55_large_record_data_50kb() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "resources", "res_055", "data").map(|s| s.len()), Some(50000));
    let _log = dump_sync_log();
}
...
fn s57_millisecond_timestamps_in_updated_at() {
    ...
    device_a.vfs_db.execute("UPDATE notes SET updated_at = ?1 WHERE id='note_057'", params![ms_ts]).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
...
fn s62_foreign_key_orphan_detection() {
    ...
    assert_eq!(get_column(&device_b.vfs_db, "notes", "note_062", "resource_id").as_deref(), Some("res_062"));
    let _log = dump_sync_log();
}
...
fn s63_self_referencing_fk_folder_parent_id() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "folders", "fld_child", "parent_id").as_deref(), Some("fld_parent"));
    let _log = dump_sync_log();
}
...
fn s64_composite_pk_record_id() {
    ...
    full_sync_cycle_mistakes(&device_a, &device_b, &cloud);
    let c: i64 = device_b.mistakes_db.query_row("SELECT COUNT(*) FROM review_session_mistakes WHERE session_id='rs_064'", [], |r| r.get(0)).unwrap();
    assert_eq!(c, 1);
    let _log = dump_sync_log();
}
...
fn s65_autoincrement_pk_on_chat_messages() {
    ...
    full_sync_cycle_mistakes(&device_a, &device_b, &cloud);
    let c: i64 = device_b.mistakes_db.query_row("SELECT COUNT(*) FROM chat_messages WHERE mistake_id='mistake_065'", [], |r| r.get(0)).unwrap();
    assert!(c >= 5);
    let _log = dump_sync_log();
}
...
fn s67_delete_parent_cascade_synced() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE folders SET deleted_at = '2026-06-01T00:00:00Z' WHERE id='fld_067'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
...
fn s68_reorder_folder_items_sort_order_preserved() {
    ...
    device_a.vfs_db.execute("UPDATE folder_items SET sort_order = 99 WHERE id='fi_068'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
...
fn s69_move_note_between_folders() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE folder_items SET folder_id = 'fld_069b' WHERE id='fi_069'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "folder_items", "fi_069", "folder_id").as_deref(), Some("fld_069b"));
    let _log = dump_sync_log();
}
...
fn s70_link_unlink_mistake_to_session_cross_db() {
    ...
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let c: i64 = device_b.chat_v2_db.query_row("SELECT COUNT(*) FROM chat_v2_session_mistakes WHERE session_id='sess_070'", [], |r| r.get(0)).unwrap();
    assert_eq!(c, 1);
    // Unlink
    device_a.chat_v2_db.execute("DELETE FROM chat_v2_session_mistakes WHERE session_id='sess_070' AND mistake_id='m_070'", []).unwrap();
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let c2: i64 = device_b.chat_v2_db.query_row("SELECT COUNT(*) FROM chat_v2_session_mistakes WHERE session_id='sess_070'", [], |r| r.get(0)).unwrap();
...
fn s71_create_todo_item_with_parent_id_subtask() {
    ...
    create_test_todo_item(&device_a, "ti_071_c", "tdl_071", "Child Task", Some("ti_071_p"));
    full_sync_cycle(&device_a, &device_b, &cloud);
    assert_eq!(get_column(&device_b.vfs_db, "todo_items", "ti_071_c", "parent_id").as_deref(), Some("ti_071_p"));
    let _log = dump_sync_log();
}
...
fn s72_pomodoro_record_with_end_time_before_start() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    let et: String = device_b.vfs_db.query_row("SELECT end_time FROM pomodoro_records WHERE id='pd_072'", [], |r| r.get(0)).unwrap();
...
fn s73_answer_submission_idempotency_client_request_id() {
    ...
    let c: i64 = device_b.vfs_db.query_row("SELECT COUNT(*) FROM answer_submissions WHERE id='as_073'", [], |r| r.get(0)).unwrap();
    assert_eq!(c, 1, "Duplicate submission should be ignored by idempotency check");
    let _log = dump_sync_log();
}
...
fn s74_question_with_parent_id_variant_question() {
    ...
    assert_eq!(get_column(&device_b.vfs_db, "questions", "q_074_v", "parent_id").as_deref(), Some("q_074_p"));
    // Also verify parent exists on B
    let parent_exists: i64 = device_b.vfs_db.query_row("SELECT COUNT(*) FROM questions WHERE id='q_074_p'", [], |r| r.get(0)).unwrap();
...
fn s75_simultaneous_folder_tree_restructure() {
    ...
    device_a.vfs_db.execute("UPDATE folders SET parent_id = 'root_b' WHERE id='child_a'", []).unwrap();
    device_b.vfs_db.execute("UPDATE folders SET deleted_at = datetime('now') WHERE id='child_a'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
...
fn s76_increment_ref_count_from_2_devices() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE resources SET ref_count = ref_count + 1 WHERE id='res_076'", []).unwrap();
    device_b.vfs_db.execute("UPDATE resources SET ref_count = ref_count + 1 WHERE id='res_076'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
...
fn s77_decrement_ref_count_verify_not_below_zero() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE resources SET ref_count = MAX(0, ref_count - 1) WHERE id='res_077'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
...
fn s78_ref_count_to_zero_should_not_delete_resource() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE resources SET ref_count = 0 WHERE id='res_078'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    let exists = row_count_all(&device_b.vfs_db, "resources");
    assert!(exists >= 1, "Resource should still exist even with ref_count=0");
    let _log = dump_sync_log();
}
...
fn s79_concurrent_pomodoro_completion_counts_sum_merge() {
    ...
    device_a.vfs_db.execute("UPDATE todo_items SET completed_pomodoros = 3 WHERE id='ti_079'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE todo_items SET completed_pomodoros = 4 WHERE id='ti_079'", []).unwrap();
    device_b.vfs_db.execute("UPDATE todo_items SET completed_pomodoros = 5 WHERE id='ti_079'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
    let cp = get_column_i64(&device_a.vfs_db, "todo_items", "ti_079", "completed_pomodoros");
...
fn s80_concurrent_attempt_count_on_question_max_merge() {
    ...
    device_a.vfs_db.execute("UPDATE questions SET attempt_count = 5 WHERE id='q_080'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE questions SET attempt_count = 7 WHERE id='q_080'", []).unwrap();
    device_b.vfs_db.execute("UPDATE questions SET attempt_count = 6 WHERE id='q_080'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
...
fn s81_concurrent_correct_count_updates_max_merge() {
    ...
    device_a.vfs_db.execute("UPDATE questions SET correct_count = 3 WHERE id='q_081'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE questions SET correct_count = 4 WHERE id='q_081'", []).unwrap();
    device_b.vfs_db.execute("UPDATE questions SET correct_count = 5 WHERE id='q_081'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
...
fn s82_concurrent_estimated_pomodoros_sum_merge() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE todo_items SET estimated_pomodoros = 3 WHERE id='ti_082'", []).unwrap();
    device_b.vfs_db.execute("UPDATE todo_items SET estimated_pomodoros = 4 WHERE id='ti_082'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
    let ep = get_column_i64(&device_a.vfs_db, "todo_items", "ti_082", "estimated_pomodoros");
...
fn s83_concurrent_total_reviews_on_review_plan_max_merge() {
    ...
    device_a.vfs_db.execute("UPDATE review_plans SET total_reviews = 10 WHERE id='rp_083'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE review_plans SET total_reviews = 12 WHERE id='rp_083'", []).unwrap();
    device_b.vfs_db.execute("UPDATE review_plans SET total_reviews = 11 WHERE id='rp_083'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
...
fn s84_ref_count_on_chat_v2_resources_separate_table() {
    ...
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    let rc: i64 = device_b.chat_v2_db.query_row("SELECT ref_count FROM resources WHERE id='cr_084'", [], |r| r.get(0)).unwrap();
    assert_eq!(rc, 3);
    let _log = dump_sync_log();
}
...
fn s85_blob_ref_count_not_in_row_sync() {
    ...
    log_sync(&format!("S85: {} blob changes collected (blobs use FileSync)", changes.len()));
    let _log = dump_sync_log();
}
...
fn s86_normal_sync_after_small_gap() {
    ...
    device_b.vfs_db.execute("UPDATE notes SET title = 'B After' WHERE id='note_086'", []).unwrap();
...
fn s87_gap_detected_and_handled() {
    ...
    log_sync(&format!("S87: has_prune_gap({}, {:?}) = {}", local_version, cloud_min, has_gap));
    assert!(has_gap);
    let _log = dump_sync_log();
}
...
fn s89_device_offline_for_10_rounds_then_reconnects() {
    ...
    for i in 0..10 {
        create_test_resource(&device_b, &format!("res_off_{}", i), "note", &format!("data{}", i), &format!("h_off_{}", i));
        create_test_note(&device_b, &format!("note_off_{}", i), &format!("res_off_{}", i), &format!("Offline Note {}", i));
    }
...
fn s90_corrupt_change_log_entry_skip_not_crash() {
    ...
    log_sync(&format!("S90: B applied {} changes (corrupt skipped)", applied));
    let c = row_count(&device_b.vfs_db, "notes");
    assert_eq!(c, 1);
    let _log = dump_sync_log();
}
...
fn s95_different_schema_versions_should_detect() {
    ...
    let a_change_log: i64 = device_a.vfs_db.query_row(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='__change_log'",
        [], |r| r.get(0)).unwrap();
    assert!(a_change_log > 0, "Device A should have __change_log table in VFS");
    // Verify device B has only chat_v2 schema (no VFS tables)
    let b_vfs_tables: i64 = device_b.vfs_db.query_row(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'",
        [], |r| r.get(0)).unwrap();
    // Device B's VFS was never set up, so it should have no tables
    assert_eq!(b_vfs_tables, 0, "Device B should have no VFS tables (only chat_v2 setup)");
    // B's chat_v2 should have tables — check for a known table
    let has_sessions: i64 = device_b.chat_v2_db.query_row(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='chat_v2_sessions'",
        [], |r| r.get(0)).unwrap();
    assert!(has_sessions > 0, "Device B should have chat_v2_sessions table");
    log_sync(&format!("S95: A has __change_log={}, B VFS tables={}, B has chat_v2_sessions={}",
        a_change_log > 0, b_vfs_tables, has_sessions > 0));
    let _log = dump_sync_log();
}
...
fn s96_ease_factor_field_merge_average() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE review_plans SET ease_factor = 2.8 WHERE id='rp_096'", []).unwrap();
    device_b.vfs_db.execute("UPDATE review_plans SET ease_factor = 2.2 WHERE id='rp_096'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
...
fn s97_interval_days_field_merge_max() {
    ...
    device_a.vfs_db.execute("UPDATE review_plans SET interval_days = 7 WHERE id='rp_097'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE review_plans SET interval_days = 10 WHERE id='rp_097'", []).unwrap();
    device_b.vfs_db.execute("UPDATE review_plans SET interval_days = 8 WHERE id='rp_097'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
...
fn s98_consecutive_failures_field_merge_max() {
    ...
    device_a.vfs_db.execute("UPDATE review_plans SET consecutive_failures = 3 WHERE id='rp_098'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE review_plans SET consecutive_failures = 5 WHERE id='rp_098'", []).unwrap();
    device_b.vfs_db.execute("UPDATE review_plans SET consecutive_failures = 4 WHERE id='rp_098'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
    let cf = get_column_i64(&device_a.vfs_db, "review_plans", "rp_098", "consecutive_failures");
...
fn s99_user_note_string_concat_merge() {
    ...
    device_a.vfs_db.execute("UPDATE questions SET user_note = 'Note from A' WHERE id='q_099'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE questions SET user_note = 'Note from A + more from A' WHERE id='q_099'", []).unwrap();
    device_b.vfs_db.execute("UPDATE questions SET user_note = 'Note from A + more from B' WHERE id='q_099'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
...
fn s100_ai_feedback_string_concat_merge() {
    ...
    device_a.vfs_db.execute("UPDATE questions SET ai_feedback = 'AI: Good work' WHERE id='q_100'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE questions SET ai_feedback = 'AI: Good work + Excellent' WHERE id='q_100'", []).unwrap();
...
fn s101_is_favorite_boolean_or_merge() {
    ...
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_a.vfs_db.execute("UPDATE notes SET is_favorite = 1 WHERE id='note_101'", []).unwrap();
    device_b.vfs_db.execute("UPDATE notes SET is_favorite = 1 WHERE id='note_101'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
...
fn s102_is_bookmarked_boolean_or_merge() {
    ...
    device_a.vfs_db.execute("UPDATE questions SET is_bookmarked = 1 WHERE id='q_102'", []).unwrap();
    full_sync_cycle(&device_a, &device_b, &cloud);
    device_b.vfs_db.execute("UPDATE questions SET is_bookmarked = 1 WHERE id='q_102'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.vfs_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.vfs_db, &for_a);
...
fn s104_citations_json_deep_merge() {
    ...
    device_a.chat_v2_db.execute("UPDATE chat_v2_blocks SET citations_json = '[{\"url\":\"a.com\"}]' WHERE id='blk_104'", []).unwrap();
    full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    ...
    let cit: String = device_a.chat_v2_db.query_row("SELECT citations_json FROM chat_v2_blocks WHERE id='blk_104'", [], |r| r.get(0)).unwrap_or_default();
...
fn s105_tags_json_set_union_on_anki_cards() {
    ...
    full_sync_cycle_mistakes(&device_a, &device_b, &cloud);
    device_a.mistakes_db.execute("UPDATE anki_cards SET tags_json = '[\"tag1\",\"tag2\"]' WHERE id='ac_105'", []).unwrap();
    device_b.mistakes_db.execute("UPDATE anki_cards SET tags_json = '[\"tag1\",\"tag3\"]' WHERE id='ac_105'", []).unwrap();
    let changes_b = collect_pending_changes(&device_b.mistakes_db);
    cloud.upload_changes(&device_b.device_id, &changes_b);
    let for_a = cloud.download_changes(&device_a.device_id);
    apply_changes_to_db(&device_a.mistakes_db, &for_a);
    let tags: String = device_a.mistakes_db.query_row("SELECT tags_json FROM anki_cards WHERE id='ac_105'", [], |r| r.get(0)).unwrap();
...
fn s106_1000_records_bulk_create_and_sync() {
    ...
    let cloud = SimulatedCloudStore::new();
    for i in 0..1000 {
        create_test_resource(&device_a, &format!("rs{}", i), "note", "d", &format!("h{}", i));
    }
    let (b_applied, _) = full_sync_cycle(&device_a, &device_b, &cloud);
    log_sync(&format!("S106: B applied {} changes for 1000 records", b_applied));
...
fn s107_100_records_each_with_50_updates() {
    ...
    let cloud = SimulatedCloudStore::new();
    for i in 0..100 {
        create_test_resource(&device_a, &format!("ru{}", i), "note", "d", &format!("hu{}", i));
    ...
    for u in 0..50 {
        for i in 0..100 {
            device_a.vfs_db.execute("UPDATE notes SET title = ?1 WHERE id = ?2", params![format!("n{}v{}", i, u), format!("nu{}", i)]).unwrap();
        }
    }
    let (b_applied, _) = full_sync_cycle(&device_a, &device_b, &cloud);
    log_sync(&format!("S107: B applied {} changes after 50x100 updates", b_applied));
...
fn s108_10_devices_each_creating_10_records() {
    ...
    for d in 0..10 {
        ...
        for i in 0..10 {
            let idx = d * 10 + i;
            create_test_resource(&dev, &format!("r10_{}", idx), "note", "d", &format!("h10_{}", idx));
        }
        devices.push(dev);
    }
    // All upload
    ...
    log_sync(&format!("S108: Device 0 applied {} total changes from 10 devices", applied));
...
fn s109_rapid_successive_syncs_10_cycles() {
    ...
    let cloud = SimulatedCloudStore::new();
    for cycle in 0..10 {
        create_test_resource(&device_a, &format!("rc{}", cycle), "note", "d", &format!("hc{}", cycle));
        full_sync_cycle(&device_a, &device_b, &cloud);
    }
    let c = row_count(&device_b.vfs_db, "resources");
...
fn s111_100_folders_with_5_items_each() {
    ...
    let cloud = SimulatedCloudStore::new();
    for f in 0..100 {
        create_test_folder(&device_a, &format!("f111_{}", f), None, &format!("Folder {}", f));
        for i in 0..5 {
            let idx = f * 5 + i;
            create_test_resource(&device_a, &format!("r111_{}", idx), "note", "d", &format!("h111_{}", idx));
            create_test_note(&device_a, &format!("n111_{}", idx), &format!("r111_{}", idx), &format!("N{}", idx));
            create_test_folder_item(&device_a, &format!("fi111_{}", idx), &format!("f111_{}", f), "note", &format!("n111_{}", idx));
        }
    }
    full_sync_cycle(&device_a, &device_b, &cloud);
...
fn s112_50_sessions_with_20_messages_each() {
    ...
    let cloud = SimulatedCloudStore::new();
    for s in 0..50 {
        create_test_chat_session(&device_a, &format!("s112_{}", s), "general_chat", &format!("Session {}", s));
        for m in 0..20 {
            let role = if m % 2 == 0 { "user" } else { "assistant" };
            create_test_chat_message(&device_a, &format!("m112_{}_{}", s, m), &format!("s112_{}", s), role, ((s * 20 + m + 1) * 1000) as i64);
        }
    }
    let (applied, _) = full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    log_sync(&format!("S112: B applied {} changes (50x20 messages)", applied));
    let sc: i64 = device_b.chat_v2_db.query_row("SELECT COUNT(*) FROM chat_v2_sessions", [], |r| r.get(0)).unwrap();
    let mc: i64 = device_b.chat_v2_db.query_row("SELECT COUNT(*) FROM chat_v2_messages", [], |r| r.get(0)).unwrap();
    assert_eq!(sc, 50);
    assert_eq!(mc, 1000);
    let _log = dump_sync_log();
}
...
fn s114_large_attachment_metadata_100_attachments() {
    ...
    for i in 0..100 {
        create_test_chat_attachment(&device_a, &format!("att_{}", i), "msg_114", &format!("file_{}.png", i), "image/png", 1024 * (i + 1) as i64);
    }
    let (applied, _) = full_sync_cycle_chat_v2(&device_a, &device_b, &cloud);
    log_sync(&format!("S114: B applied {} changes (100 attachments)", applied));
    let c: i64 = device_b.chat_v2_db.query_row("SELECT COUNT(*) FROM chat_v2_attachments WHERE message_id='msg_114'", [], |r| r.get(0)).unwrap();
    assert_eq!(c, 100);
    let _log = dump_sync_log();
}
...
fn s115_full_database_population_all_tables_sync() {
    ...
    create_test_chat_attachment(&device_a, "att_full", "msg_full", "img.png", "image/png", 1024);
    ...
    create_test_review_session(&device_a, "rs_full", "Full Session", "2026-05-01", "2026-05-07");
    ...
    for (table, db_name) in &checks {
        assert!(verify_devices_converged(&device_a, &device_b, table, db_name),
            "Table {}.{} should converge", db_name, table);
    }
