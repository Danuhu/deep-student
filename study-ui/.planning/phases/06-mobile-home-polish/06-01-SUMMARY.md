...
- `rg --files src | rg '(\\.source\\.test|\\.test)\\.tsx?$' | xargs node --test --experimental-strip-types` - passed, 225 tests.
- `npm run lint` - passed.
- `npm run build` - passed, Vite built 4700 modules.
- Playwright viewport smoke passed for `390x844`, `768x1024`, and `1280x800`; screenshots are recorded in `06-UAT.md`.
