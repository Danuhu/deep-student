...
describe('chat v2 conversation control entry contract', () => {
  ...
  const plusMenuSource = readFileSync(resolve(process.cwd(), 'src/features/chat/components/input-bar/ComposerPlusMenu.tsx'), 'utf-8');
  ...
  it('moves the desktop conversation-control trigger into the input bar', () => {
    ...
    expect(plusMenuSource).toContain('SlidersHorizontal');
    expect(plusMenuSource).toContain("{t('common:chat_controls')}");
