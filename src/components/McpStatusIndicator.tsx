...
const McpStatusIndicator: React.FC<{ compact?: boolean }> = ({ compact }) => {
  ...
  if (compact) {
    return (
      <DsButton variant="ghost" size="sm" onClick={load} title={t('mcpStatus.refreshStatus')} className={`!h-auto text-xs ${textClass} hover:text-foreground`}>
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <DsButton variant="ghost" size="sm" onClick={() => { McpService.connectAll().catch((err) => { debugLog.warn('[MCP] Connect failed:', err); }); load(); }} title={t('actions.refresh')} className="!px-2 !py-1 !h-auto text-xs border border-border text-muted-foreground hover:bg-[var(--interactive-hover)]">
