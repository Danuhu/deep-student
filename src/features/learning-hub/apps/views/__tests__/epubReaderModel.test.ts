...
async function createMinimalEpub(): Promise<string> {
  ...
  zip.file('OPS/text/chapter-1.xhtml', '<html><head><title>One</title></head><body><h1>Chapter One</h1><p>A searchable phrase.</p><script>window.bad = true</script></body></html>');
  zip.file('OPS/text/chapter-2.xhtml', '<html><body><h1 id="part">Chapter Two</h1><p>More searchable text.</p></body></html>');
...
describe('epubReaderModel', () => {
  ...
  it('searches all spine chapters and sanitizes active content', async () => {
    ...
    const rendered = await renderEpubChapter(book, 0, 'dark', 1.2);
