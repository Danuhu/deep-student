            ...
            {!isCodexOAuthVendor && <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto">
              ...
              {isEditingVendor ? (
                <>
                  ...
                  <DsButton size="sm" variant={isSmallScreen ? 'outline' : 'primary'} className={cn('min-h-11 flex-1 sm:min-h-0 sm:flex-none [@media(pointer:coarse)]:!min-h-11', isSmallScreen && 'text-primary')} onClick={handleSaveEditVendor} disabled={vendorSaving}>{t('common:actions.save')}</DsButton>
                </>
              ) : (
                <>
                  <DsButton size="sm" variant={isSmallScreen ? 'outline' : 'ghost'} className="min-h-11 flex-1 sm:min-h-0 sm:flex-none [@media(pointer:coarse)]:!min-h-11" onClick={() => handleStartEditVendor(selectedVendor)}>{t('common:actions.edit')}</DsButton>
                  {!selectedVendorIsSiliconflow && !selectedVendor.isBuiltin && !selectedVendor.isReadOnly && (
                    ...
                    <DsButton
                      ...
                      className={cn('min-h-11 flex-1 sm:min-h-0 sm:flex-none [@media(pointer:coarse)]:!min-h-11 [@media(pointer:coarse)]:!min-w-11', isSmallScreen && confirmingDelete?.type !== 'vendor' && '!text-destructive', isSmallScreen && confirmingDelete?.type === 'vendor' && 'whitespace-nowrap')}
                      onClick={() => {
                        if (!isSmallScreen) {
          ...
          {!selectedVendorIsSiliconflow && (
            ...
            >
              ...
              <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto sm:flex-shrink-0">
                ...
                {/* 移动端去纯色大按钮：描边+主色文字，桌面保持实心 primary */}
                <DsButton size="sm" variant={isSmallScreen ? 'outline' : 'primary'} className={cn('min-h-11 flex-1 sm:min-h-0 sm:flex-none [@media(pointer:coarse)]:!min-h-11', isSmallScreen && 'text-primary')} onClick={() => {
                  if (usePanelModelEditor) {
                    if (isAddingNewModel) handleCancelAddModel();
                    setInlineEditState(null);
