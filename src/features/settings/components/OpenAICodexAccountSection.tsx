    ...
    >
      ...
      {statusLoading && !status ? (
        ...
        <div className="flex flex-wrap items-center gap-2 border-t border-border/30 px-4 py-3 max-md:justify-end">
          ...
          {state === 'reauth_required' && (
            <DsButton size="sm" variant="outline" className="[@media(pointer:coarse)]:!min-h-11" onClick={() => void handleLogout()} disabled={disabled}>
              {busyAction === 'logout' ? <Spinner className="h-3.5 w-3.5 animate-spin" /> : <SignOut className="h-3.5 w-3.5" />}
              {translate(t, 'remove_account', 'Remove account')}
            </DsButton>
          )}
          {!status && (
            <DsButton size="sm" variant="outline" className="[@media(pointer:coarse)]:!min-h-11" onClick={() => void refreshStatus(true)} disabled={disabled}>
        ...
        <div className="space-y-3 border-t border-border/30 px-4 py-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {login?.authUrl && (
              <DsButton size="sm" variant="outline" className="[@media(pointer:coarse)]:!min-h-11" onClick={() => void openUrl(login.authUrl!)}>
                <ArrowSquareOut className="h-3.5 w-3.5" />
                {translate(t, 'reopen_sign_in', 'Reopen sign-in page')}
              </DsButton>
            )}
            {login?.verificationUrl && (
              <DsButton size="sm" variant="outline" className="[@media(pointer:coarse)]:!min-h-11" onClick={() => void openUrl(login.verificationUrl!)}>
                <ArrowSquareOut className="h-3.5 w-3.5" />
