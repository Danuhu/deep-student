...
const normalizeSchedule = (schedule: AutomationSchedule): AutomationSchedule => ({
  ...
  ...(schedule.kind === 'weekly' ? { weekday: schedule.weekday ?? 1 } : {}),
...
const validateDraft = (draft: FormDraft): string | null => {
  ...
  if (draft.schedule.kind !== 'interval' && draft.schedule.timezone) {
    try {
...
const isVersionConflict = (cause: unknown) =>
  getErrorDetails(cause).code === AUTOMATION_VERSION_CONFLICT_CODE;
...
export const AutomationSettingsSection: React.FC<AutomationSettingsSectionProps> = ({
  ...
  useEffect(() => {
    ...
    const stopSync = startAutomationSync();
    void refresh();
    return stopSync;
    // refresh 来自 store，引用稳定；仅在桌面可用性变化时重建同步。
  ...
  const displayError = useMemo(() => {
    ...
    if (error.includes(AUTOMATION_VERSION_CONFLICT_CODE)) return actionError;
        ...
        >
          <div className="min-w-0 flex-1 space-y-1">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              ...
              <span className="shrink-0 rounded border border-border bg-muted/60 px-1.5 py-0.5 text-[10px] text-muted-foreground">
          ...
          >
            ...
            <NotionButton
              ...
              size="icon"
              iconOnly
              className={cn('!h-7 !w-7', expanded && 'bg-primary/10 text-primary')}
    ...
    >
      ...
      {desktopUnavailable ? (
        ...
        <>
          ...
          {embedded && !loading && (
            <p className="text-right text-[11px] tabular-nums text-muted-foreground/70">
              {t('settings:automation.capacity', { count, max })}
            </p>
          )}
        </>
