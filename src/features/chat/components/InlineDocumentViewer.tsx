    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5 border-b border-border bg-muted/50 px-3 py-2">
        ...
        <div className={cn('flex items-center gap-1', isSmallScreen && 'min-w-0 flex-1')}>
          ...
          <NotionButton variant="ghost" size="icon" iconOnly disabled={matches.length === 0} onClick={() => gotoMatch(-1)} className="bg-muted hover:bg-[var(--interactive-hover)] disabled:opacity-40" aria-label={t('chatV2:documentViewer.prevMatch')} title={t('chatV2:documentViewer.prevMatch')}>
            <CaretUp size={14} />
          </NotionButton>
          <NotionButton variant="ghost" size="icon" iconOnly disabled={matches.length === 0} onClick={() => gotoMatch(1)} className="bg-muted hover:bg-[var(--interactive-hover)] disabled:opacity-40" aria-label={t('chatV2:documentViewer.nextMatch')} title={t('chatV2:documentViewer.nextMatch')}>
            <CaretDown size={14} />
          </NotionButton>
        </div>
