...
import type { ClawHubSkillCard, SkillUpdateCheckResult } from './api';

/** ClawHub 列表区 UI 状态（不含 loading 过程中的瞬时态时可由调用方叠加） */
export type ClawHubListUiStatus =
  | 'idle'
  | 'loading'
  | 'success'
  | 'empty'
...
export type ClawHubSearchFailureKind = 'rate_limited' | 'network_error';

/**
 * 将 invoke / 网络异常归类为限流或网络错误（其余一律按网络错误展示）。
 * 保持 hermetic：只看错误字符串，不发起网络请求。
 */
export function classifyClawHubSearchError(err: unknown): ClawHubSearchFailureKind {
...
export function resolveClawHubSearchSuccess(
  items: readonly ClawHubSkillCard[],
): 'success' | 'empty' {
...
/**
 * 更新漂移文案：clawhub 用完整 version；url/tap 用 sha256 短前缀。
